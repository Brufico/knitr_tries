---
title: "Pdf (Latex) tries for using a template"
shorttitle: no
subtitle : "Une sorte d'exploration pour comprendre et utiliser un template"
author: 
- Bruno Fischer Colonimos
- Aristote, Bruno
- Brufico, FischerCol
shortauthor: Bruno et al.
institute: Ipag, www.ipag.fr 
abstract: |
    This is just an experiment on formatting with a new template.  
    I would certainly drag it for a long time, in order to cover many lines. But it will certainly be BORING,
    
    BORING, BORING, BORING, BORING, BORING, BORING, BORING, BORING  
    BORING, BORING, BORING, BORING, BORING, BORING, BORING, BORING. link = www.ipag.fr
    
documentclass: article
papersize: a4
fontsize: 12pt

lang: fr # en = english, fr = français
toc-sommaire: yes # si lang = fr et si la table est au début du doc, yes u true
# nom de la table = "Sommaire"

debug_var: yes # pour afficher les variables dans le texte
# lang: english # francais # english ********************This is the problem

# classoption: a4paper
# header-includes: \usepackage{fancyhdr}\pagestyle{fancy} \fancyfoot[C]{\thetitle \ \textemdash \ page \thepage}
# header-includes:
# - \usepackage{fancyhdr}
# - \pagestyle{fancy}
# - \fancyfoot[CE,CO]{\theauthor \ --\ \thedoctitle} # thetitle ne marche pas et donne le n0 de section
# - \fancyfoot[LE, RO]{\thepage}

footer: \theshortauthor \ --\ \theshorttitle \ --\ page\ \thepage #????
# babel: english # francais # english
# secnumdepth: 4
output: 
    pdf_document:
      toc: yes            
      number_section: true
      template: ./common/ptemplate3.tex
urlcolor: "blue"
linkcolor: "red"
toccolor: "red"
# font-family:
# font-family: "Arial"
# mainfont: Helvetica


# a4paper, 
geometry: "left=2.2cm,right=2.2cm,top=2cm,bottom=2cm,headsep=0.5cm,footskip=1cm"
subparagraph: yes  # nécessaire avec titlesec sinon erreur avec paragraph et subp
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE, fig.pos = "H")
```

************************

Lorem ipsum{#lorem} 
===========

Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Maecenas porttitor congue massa. Fusce posuere, magna sed pulvinar ultricies, purus lectus malesuada libero, sit amet commodo magna eros quis urna. Nunc viverra imperdiet enim. Fusce est.

Vivamus{-}
-------
Vivamus a tellus. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Proin pharetra nonummy pede. Mauris et orci. Aenean nec lorem.

Porttitor
---------
In porttitor. Donec laoreet nonummy augue. Suspendisse dui purus, scelerisque at, vulputate vitae, pretium mattis, nunc. Mauris eget neque at sem venenatis eleifend. Ut nonummy.
Fusce aliquet pede non pede. Suspendisse dapibus lorem pellentesque magna. Integer nulla. Donec blandit feugiat ligula. Donec hendrerit, felis et imperdiet euismod, purus ipsum pretium metus, in lacinia nulla nisl eget sapien.

Donec
-----
Donec ut est in lectus consequat consequat. Etiam eget dui. Aliquam erat volutpat. Sed at lorem in nunc porta tristique. Proin nec augue.

### Quisque

Quisque aliquam tempor magna. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. 

#### Nunc
Nunc ac magna. Maecenas odio dolor, vulputate vel, auctor ac, accumsan id, felis.

#### Pellentesque cursus
Pellentesque cursus sagittis felis.

##### Vivamus{-}  
\ a mi. Fusce in sapien eu purus dapibus commodo. 

##### Morbi
\ neque. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus.

##### Aliquam
\ erat volutpat.(cf [Integer](#Integer))


### Pellentesque

Pellentesque porttitor, velit lacinia egestas auctor, diam eros tempus arcu, nec vulputate augue magna vel risus. Cras non magna vel ante adipiscing rhoncus. Vivamus a mi. Morbi neque. Aliquam erat volutpat. link =  www.ipag.fr 


Integer ultrices lobortis eros {#Integer}
==============================

Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Proin semper, ante vitae sollicitudin posuere, metus quam iaculis nibh, vitae scelerisque nunc massa eget pede. Sed velit urna, interdum vel, ultricies vel, faucibus at, quam. Donec elit est, consectetuer eget, consequat quis, tempus quis, wisi.
In in nunc. Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos hymenaeos. Donec ullamcorper fringilla eros. Fusce in sapien eu purus dapibus commodo. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus.

Cras faucibus condimentum odio. Sed ac ligula. Aliquam at eros. Etiam at ligula et tellus ullamcorper ultrices. In fermentum, lorem non cursus porttitor, diam urna accumsan lacus, sed interdum wisi nibh nec nisl. (see [Lorem](#lorem) )

